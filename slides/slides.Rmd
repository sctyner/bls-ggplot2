...
```{r, echo=TRUE, out.width="80%", fig.height = 3.5}
dat = read_tsv("dat/gdpr_violations.tsv")
...
```{r echo=TRUE,out.width="80%", fig.height = 3.5}
ggplot(dat, aes(x = name, y = log(price+1))) + 
  geom_boxplot(fill = "green") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  labs(x = "EU Country", title = "Boxplot for log(price+1) of GDPR violations or EU Countries")
```
...
library(sf)
gdpr_countries <- sort(unique(dat$name))
world <- rnaturalearth::ne_countries(scale = "medium", returnclass = "sf")
gdpr_countries <- world %>% filter(name_long %in% gdpr_countries)
country_fines <- dat %>% group_by(name) %>% 
  summarize(total_fines = sum(price))
gdpr_countries <- left_join(gdpr_countries, country_fines, 
                            by = c("name_long"= "name"))
scale_labs <- c("€100,000", "€1,000,000", "€10,000,000", "€100,000,000")
extrafont::loadfonts(quiet=TRUE)
ggplot() + 
  geom_sf(data = world, fill = "grey90") + 
  geom_sf(data = gdpr_countries, aes(fill = log10(total_fines))) + 
  coord_sf(xlim = c(-22, 38), ylim = c(32, 71)) + 
  scale_fill_binned(name = "Total fines levied\n(log10 scale)", 
                    low = "white", high = "forestgreen", 
                    labels = scale_labs) + 
  labs(title = "Fines by country where violation was enforced", x = NULL, y = NULL) +
  theme_minimal() + 
  theme(plot.background = element_rect(fill = "#deebf7", color = NA),
        text = element_text(family = "Gentium"), 
        axis.text = element_blank(), legend.position = c(.1, .15), 
        legend.background = element_rect(fill = "#deebf7", color = NA))
```
...
```{r echo=FALSE, fig.height=6, out.width="50%"}
# install.packages("rnaturalearth")
library(sf)
gdpr_countries <- sort(unique(dat$name))
world <- rnaturalearth::ne_countries(scale = "medium", returnclass = "sf")
